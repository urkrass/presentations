...
<style scoped>
.instrument-scene{display:grid;grid-template-columns:1.18fr .82fr;gap:46px;min-height:500px;align-items:stretch}.instrument-image{position:relative;margin:0;height:500px;overflow:hidden;border-radius:var(--radius-field);background:#151515}.instrument-image img{width:100%;height:100%;object-position:center;filter:saturate(.9);transition:opacity var(--motion-reveal) var(--ease),filter var(--motion-reveal) var(--ease)}.instrument-image figcaption{position:absolute;left:20px;bottom:16px;padding:8px 11px;border-radius:var(--radius-control);background:rgba(18,18,18,.76);color:#fff;font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}.instrument-copy{display:grid;align-content:center;gap:18px}.instrument-copy h2{margin:0;font:500 34px/1.12 Georgia,serif}.instrument-copy>p:not(.scene-kicker){margin:0;color:var(--muted);font-size:18px;line-height:1.45}.instrument-limit{display:grid;gap:5px;padding:16px 17px;border-radius:var(--radius-field);background:var(--paper-deep);color:var(--ink)!important}.instrument-limit strong{color:var(--rust);font-size:12px;letter-spacing:.1em;text-transform:uppercase}.scene-switcher{display:flex;gap:8px}.scene-switcher button{padding:10px 14px;border:0;border-radius:var(--radius-control);background:var(--paper-deep);color:var(--muted);font-weight:750;cursor:pointer;text-transform:capitalize}.scene-switcher button.active{background:var(--blue);color:#fff}button:focus-visible{outline:3px solid color-mix(in srgb,var(--blue) 48%,transparent);outline-offset:3px}
</style>
