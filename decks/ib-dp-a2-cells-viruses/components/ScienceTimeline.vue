...
<template>
  <section class="science-timeline">
    <figure :class="{ contain: event.fit === 'contain' }">
      <img :src="event.image" :alt="event.alt" :style="{ objectPosition: event.position || 'center' }" />
      <figcaption>{{ event.date }} · {{ event.name }}</figcaption>
    </figure>
    <div class="timeline-copy">
      ...
      <div class="timeline-dates" role="list" aria-label="Choose a historical stage">
        <button v-for="(item,index) in events" :key="item.date" type="button" :class="{active:index===active}" @click="active=index">
...
<style scoped>
.science-timeline{display:grid;grid-template-columns:1.12fr .88fr;gap:46px;min-height:500px}.science-timeline figure{position:relative;margin:0;height:500px;overflow:hidden;background:#161616}.science-timeline img{width:100%;height:100%;object-fit:cover;filter:saturate(.82)}.science-timeline figure.contain img{object-fit:contain;background:#efe6d3}.science-timeline figcaption{position:absolute;left:18px;bottom:16px;padding:8px 11px;background:rgba(12,12,12,.72);color:#fff;font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.timeline-copy{display:grid;align-content:center;gap:17px}.timeline-copy h2{margin:0;font:500 35px/1.12 Georgia,serif}.timeline-copy>p:not(.scene-kicker){margin:0;color:var(--muted);font-size:17px;line-height:1.44}.timeline-copy .consequence{padding:15px 17px;background:var(--soft-green);color:var(--green)!important;font-family:Georgia,serif;font-size:19px!important}.timeline-dates{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.timeline-dates button{min-height:68px;padding:11px 10px;border:0;background:var(--paper-deep);color:var(--muted);text-align:left;cursor:pointer}.timeline-dates button strong,.timeline-dates button span{display:block}.timeline-dates button strong{color:var(--rust);font:500 19px/1 Georgia,serif}.timeline-dates button span{margin-top:6px;font-size:11px;font-weight:800;text-transform:uppercase}.timeline-dates button.active{background:var(--ink);color:#fff}.timeline-dates button.active strong{color:#efb79f}button:focus-visible{outline:3px solid color-mix(in srgb,var(--blue) 48%,transparent);outline-offset:3px}
</style>
