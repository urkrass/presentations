...
import { moleLegacyRedirectScript, writeMoleLegacyRedirect } from './lib/mole-route-alias.mjs'
...
buildDeck('decks/grade-8-empirical-formula/slides.md', publicPath('/grade-8/empirical-formula/'), path.join(dist, 'grade-8', 'empirical-formula'))
buildDeck('decks/ib-dp-mole-history/slides.md', publicPath('/ib-dp/mole-history/'), path.join(dist, 'ib-dp', 'mole-history'))
...
addRouteRecovery(path.join(dist, 'grade-8', 'empirical-formula'))
addRouteRecovery(path.join(dist, 'ib-dp', 'mole-history'))
...
const notFound = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Opening presentation…</title></head><body><p>Opening presentation…</p><script>(()=>{${moleLegacyRedirectScript(siteBase)};const bases=${JSON.stringify(publicDeckRoutes)};const base=bases.find(item=>location.pathname.startsWith(item));if(base){const target=location.pathname+location.search+location.hash;location.replace(base+'?__slidev_redirect='+encodeURIComponent(target))}else{location.replace(${JSON.stringify(`${siteBase}/`)})}})()</script></body></html>`
fs.writeFileSync(path.join(dist, '404.html'), notFound)
writeMoleLegacyRedirect(root, siteBase)
