...
export function freshState(){return {format:'biology-workbook',version:1,worksheetId:WORKSHEET_ID,student:{name:'',className:'',date:new Date().toLocaleDateString('en-CA')},answers:{},lastPage:'start',updatedAt:null};}
...
export function validateState(data){if(!obj(data)||data.format!=='biology-workbook'||data.version!==1||data.worksheetId!==WORKSHEET_ID)throw Error('Choose a Viruses HL workbook backup (.json).');if(!obj(data.student)||!obj(data.answers))throw Error('This backup is incomplete.');const s=freshState();s.student={name:str(data.student.name,100),className:str(data.student.className,60),date:str(data.student.date,10)};for(const [id,a] of Object.entries(data.answers)){if(!questions.some(q=>q.id===id)||!obj(a))throw Error('Unknown or invalid response.');s.answers[id]={text:str(a.text),math:str(a.math),chem:str(a.chem),drawing:strokes(a.drawing)};}s.lastPage=pages.some(p=>p.id===data.lastPage)?data.lastPage:'start';s.updatedAt=typeof data.updatedAt==='string'?data.updatedAt:null;return s;}
