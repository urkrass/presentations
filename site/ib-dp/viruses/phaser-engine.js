...
function textures(scene){
 ...
 // Pre-colour cell textures: Phaser's per-sprite tint is WebGL-only. These
 // bitmap variants keep infection cues identical in the Canvas fallback.
 for(const [key,tint] of [['cell-infected','rgba(210,80,145,.3)'],['cell-assembly','rgba(205,43,116,.45)']])make(key,256,144,c=>{c.drawImage(scene.textures.get('cell').getSourceImage(),0,0);c.globalCompositeOperation='source-atop';c.fillStyle=tint;c.fillRect(0,0,256,144);c.globalCompositeOperation='source-over';});
...
export function createEngine(host,{getWorld,onFrame,onInspect,onReady,onError,forceCanvas=false}){
 ...
 class CultureScene extends Phaser.Scene{
  ...
  drawWorld(w){
   ...
   if(w.name==='lambda'){
    for(const c of w.cells){const key='cell:'+c.id;seen.add(key);const obj=sprite(this,key,'cell',c.x,c.y,100,57);obj.setRotation(c.angle).setTexture(c.status==='assembly'?'cell-assembly':['infected','entry'].includes(c.status)?'cell-infected':'cell');
